...
public class RunTestsExternal {
	...
	protected static void runTest(String testName, boolean verbose) {
		RunTestsExternal.verbose = verbose;
		runTest(testName);
	}
	...
	protected static String[] buildArgs(String testName) {
        ...
        // JastAdd options
		File jaFile = new File("./jastadd2.options");
		if(jaFile.exists()) {
			try {
				StreamTokenizer st = new StreamTokenizer(new FileInputStream(jaFile));
				st.resetSyntax();
				st.wordChars('a', 'z');
				st.wordChars('A', 'Z');
				st.wordChars(128 + 32, 255);
